...
public:
    ...
    const v_d& processFrame(int16_t* samples, size_t N) {
    ...
    void set_prev_samples(int16_t* prev_samples, size_t N) {
      prevsamples.assign(prev_samples, prev_samples + N);
    }
};

const float micro_seconds = 1000.0;
const int cepstra = 40;
const int filters = 40;
const int win_len = 50;
const int shift_len = 20;
const double low_freq = 50;

void mfcc(int sample_rate, short* in_array, int size_in, double** out_array, int* dim1, int* dim2) {
  int output_length = (int)((size_in * micro_seconds / sample_rate - win_len) / shift_len) + 1;

  *dim1 = output_length;
  *dim2 = cepstra + 1;
  int win_samples = win_len * sample_rate / 1e3;
  int shift_samples = shift_len * sample_rate / 1e3;
  *out_array = (double*)malloc(sizeof(double) * (*dim1) * (*dim2));

  // Set inital prev-samples
  MFCC mfcc(sample_rate, cepstra, win_len, shift_len, filters, low_freq, sample_rate / 2);
  mfcc.set_prev_samples(in_array, (win_samples - shift_samples));
  in_array += (win_samples - shift_samples);

  // Process frames one by one
  for (int index = 0; index < output_length; index++) {
    double* row = *out_array + (index * (*dim2));
    const v_d& result = mfcc.processFrame(in_array, shift_samples);
    for (int pos = 0; pos < result.size(); pos++) {
      row[pos] = result[pos];
    }
    in_array += shift_samples;
  }
}
